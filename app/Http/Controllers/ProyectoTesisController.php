...
class ProyectoTesisController extends Controller
{
    public function index(){
        ...
        $tinvestigacion = TipoInvestigacionCurso::all();
        ...
        return view('cursoTesis20221.cursoTesis',['autor'=>$autor,
                ...
                'referencias'=>$referencias, 'detalleHistorial'=>$detalleHistorial
            ]);
    }
    ...
    public function saveTesis(Request $request){
        ...
        try {
            ...
            if($request->txtreal_problematica!=""){$tesis->real_problematica = $unirtxtAreaRP;}
            if($request->txtantecedentes!=""){$tesis->antecedentes = $unirtxtAreaANT;}
            if($request->txtjustificacion!=""){$tesis->justificacion = $unirtxtAreaJI;}
            if($request->txtformulacion_prob!=""){$tesis->formulacion_prob = $unirtxtAreaFP;}
    ...
    public function showEstudiantes(){
        ...
        $estudiantes = DB::table('estudiante_ct2022')->join('asesor_curso','estudiante_ct2022.cod_docente','=','asesor_curso.cod_docente')
                            ...
                            ->select('estudiante_ct2022.*','tesis_ct2022.estado')->where('asesor_curso.username','=',auth()->user()->name)->get();
